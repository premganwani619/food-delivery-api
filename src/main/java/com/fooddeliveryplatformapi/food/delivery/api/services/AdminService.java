...
import com.fooddeliveryplatformapi.food.delivery.api.models.*;
...
import com.fooddeliveryplatformapi.food.delivery.api.repositories.IAdminRepo;
import com.fooddeliveryplatformapi.food.delivery.api.services.hashingutility.PasswordEncryptor;
    ...
    public List<Order> getAllOrders(String email, String tokenValue) {
        if (adminAuthService.authenticate(email, tokenValue)) {
            ...
            for (Order order : orderList) {
                User user = new User(order.getUser().getUserId(), order.getUser().getUserName(), null, null, order.getUser().getAddress());
                order.setUser(user);
